<template lang="">
    <div class="search-result" :class="{'result-modal':!config.SEARCH_BUTTON}">
        ...
        <div v-if="searchResults.length>0">
            <question-result-item v-for="item in searchResults" :key="item.id" :data="item" type="result"/>
        </div>
...
import QuestionResultItem from "@/components/settings/QuestionResultItem.vue";
...
export default class SearchResultManually extends mixins(PageMixin) {
  get unSelectedTags() {
