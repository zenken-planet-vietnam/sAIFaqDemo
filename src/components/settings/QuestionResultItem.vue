<template>
    <div class="question-item">
        <div class="question-content">
          ...
          <high-light :textColor="'rgba(14,183,84,0.21)'" :text="data.label" :targets="getTargets" v-else/>

        </div>
        <div>
            <b-button
                v-if="!isPromotingQuestion(data)" variant="outline-success"
                @click="promoteQuestionManual(data)" size="sm" class="mr-1"
                title="Promote this result"
            >
              <feather-icon icon="TrendingUpIcon"></feather-icon>
            </b-button>
            <b-button
                v-else variant="outline-danger"
            ...
            <b-button
                v-if="!isHidingQuestion(data)" variant="outline-danger"
                @click="hideQuestionManual(data)" size="sm"
                title="Hide this result"
            >
              <feather-icon icon="EyeOffIcon"></feather-icon>
            </b-button>
            <b-button
                v-else variant="outline-primary"
                @click="unpinQuestion(data)" size="sm"
...
export default class QuestionResultItem extends mixins(SettingMixin) {
  ...
  promoteQuestionManual(data: any) {
    if (this.isHidingQuestion(data))
      this.unpinQuestion(data)
      .then(() => this.promoteQuestion(data))
    else
      this.promoteQuestion(data)
  }

  hideQuestionManual(data: any) {
    if (this.isPromotingQuestion(data))
      this.unpinQuestion(data)
    .then(() => this.hideQuestion(data) )
    else
      this.hideQuestion(data)
  }
